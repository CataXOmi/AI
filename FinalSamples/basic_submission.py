...
def tokenize(text):
    ...
    # result = [i for i in tokens if not i in stop_words if len(i) > 2]
...
for idx in range(0, data.shape[1]):
    if np.sum(data[idx]) != 0:
        data[idx] = data[idx]/(np.sum(data[idx])) #math.sqrt(np.sum(data[idx]))

for idx in range(0, test_data.shape[0]):
    if np.sum(test_data[idx]) != 0:
        test_data[idx] = test_data[idx]/(np.sum(test_data[idx])) #math.sqrt(np.sum(test_data[idx])) 
'''
...
scoruri = []
i=1
for train, valid, y_train, y_valid in cross_validate(20, data, labels):
    clf.fit(train, y_train)
    predictii = clf.predict(valid)
...
'''
# KNN SERGIU
# Luati bucatile de cod de mai jos si calculati acc sau F1(mai bine)
# medie dupa mai mult de 10 rulari de train/test split
acc_list = []
for i in range(10):
    train, valid, y_train, y_valid = split(data, labels)
    clf = KNN_classifier(train, y_train)
    predictii = clf.classify_images(valid, k=71, metric='l1')
    precision, recall = precision_recall_score(y_valid, predictii)
    f1 = (2*precision*recall)/(precision+recall)
    print('Testarea', i, 'are', f1)
    acc_list.append(f1)
...
preds= clf.classify_images(test_data).astype('int')
print(preds)
